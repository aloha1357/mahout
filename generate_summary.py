...
def parse_ncu_csv(file_path):
    ...
    for name, values in metrics.items():
        try:
            ...
            numeric_values = [
                float(v.replace(",", "")) for v in values if v not in ["nan", "n/a", ""]
            ]
            if numeric_values:
...
def generate_summary():
    ...
    with open("AdaptiveGEMM_repo/reports/ncu_summary_new.txt", "w") as out:
        ...
        for label, path in files.items():
            ...
            if data:
                out.write(
                    f"  DRAM throughput: {data.get('dram__throughput.avg.pct_of_peak_sustained_elapsed', 'N/A')}\n"
                )
                out.write(
                    f"  L2 throughput: {data.get('lts__throughput.avg.pct_of_peak_sustained_elapsed', 'N/A')}\n"
                )
                out.write(
                    f"  SM throughput: {data.get('sm__throughput.avg.pct_of_peak_sustained_elapsed', 'N/A')}\n"
                )
                out.write(
                    f"  Warps active: {data.get('smsp__warps_active.avg.pct_of_peak_sustained_active', 'N/A')}\n"
                )
                out.write(
                    f"  Registers / thread: {data.get('launch__registers_per_thread', 'N/A')}\n"
                )
                out.write(
                    f"  Shared memory / block: {data.get('launch__shared_mem_per_block', 'N/A')}\n"
                )
                out.write(
                    f"  Kernel duration (ns): {data.get('gpu__time_duration.sum', 'N/A')}\n"
                )
                if "sm__pipe_tensor_active.avg.pct_of_peak_sustained_active" in data:
                    out.write(
                        f"  Tensor Core active: {data.get('sm__pipe_tensor_active.avg.pct_of_peak_sustained_active', 'N/A')}\n"
                    )
            else:
