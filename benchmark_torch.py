...
def benchmark_our_engine(n_qubits, batch_size):
    ...
    try:
        ...
        result = subprocess.run([executable, str(n_qubits), str(batch_size)], 
...
def main():
    ...
    print(f" Batch Size: {BATCH_SIZE} | Precision: FP64 (Simulated via INT8 TC for Our Engine)")
    print("="*85)
    print(f"{'N (Qubits)':<10} | {'Matrix Size':<12} | {'PyTorch Eager':<16} | {'PyTorch Compiled':<18} | {'AdaptiveGEMM (Our)'}")
    print("-"*85)
    ...
    for n in QUBIT_RANGE:
        ...
        print(f"{n:<10} | {size_str:<12} | {pt_eager:<16} | {pt_comp:<18} | {our_score}")

    print("="*85)
    print("Note: N=16 requires a 32GB FP64 matrix, which OOMs on most consumer GPUs.")
    print("      AdaptiveGEMM succeeds because it is Matrix-Free (Kronecker Decomposition).")
    print("="*85 + "\n")
