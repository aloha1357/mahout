...
The experimental **`apache_mout` repo (`origin`)** is the right place to run
**realistic** benchmarks—not only micro-benchmarks like `benchmark_phase.py`.
See [BENCHMARK_HANDOVER_APACHE_MOUT.md](BENCHMARK_HANDOVER_APACHE_MOUT.md) for file
manifest and checkout commands.
...
|------|-------------------|
| Build & test | `pr6-tensor-core-acceleration` on `origin` (`apache_mout`) |
