...
impl IqpEncoder {
    ...
    /// Encode multiple IQP samples using Native FWT (bypasses Ozaki / INT8 TC)
    #[cfg(target_os = "linux")]
    pub fn encode_batch_native(
        ...
        let batch_state_vector = {
            crate::profile_scope!("GPU::AllocBatchNative");
            GpuStateVector::new_batch(device, num_samples, num_qubits, Precision::Float64)?
        };

        let input_bytes = std::mem::size_of_val(batch_data);
        let data_gpu = {
            crate::profile_scope!("GPU::H2D_BatchIqpDataNative");
            device.htod_sync_copy(batch_data).map_err(|e| {
                map_allocation_error(input_bytes, "IQP Native batch upload", Some(num_qubits), e)
            })?
        };
        ...
        {
            crate::profile_scope!("GPU::BatchKernelLaunchNative");
            let ret = unsafe {
                qdp_kernels::launch_iqp_encode_native(
                    *data_gpu.device_ptr() as *const f64,
                    state_ptr as *mut c_void,
                    num_samples,
                    state_len,
            ...
            if ret != 0 {
                return Err(MahoutError::KernelLaunch(format!(
                    "Batch IQP Native encoding kernel failed: {} ({})",
                    ret,
                    cuda_error_to_string(ret)
                )));
            }
        ...
        {
            crate::profile_scope!("GPU::SynchronizeNative");
            device
                .synchronize()
                .map_err(|e| MahoutError::Cuda(format!("Sync failed: {:?}", e)))?;
        }
