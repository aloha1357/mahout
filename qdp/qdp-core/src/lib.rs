...
impl QdpEngine {
    ...
    /// Encode a batch of IQP samples via the Native FWT path (no Ozaki quantization).
    #[cfg(target_os = "linux")]
    pub fn encode_batch_native(
        &self,
        batch_data: &[f64],
        num_samples: usize,
        sample_size: usize,
        ...
        crate::profile_scope!("Mahout::EncodeBatchNative");
        ...
        let encoder = match encoding {
            ...
            _ => {
                return Err(MahoutError::InvalidInput(format!(
                    "encode_batch_native supports iqp and iqp-z only, got {}",
                    encoding_method
                )));
            }
        };
        let state_vector = encoder.encode_batch_native(
            &self.device,
            batch_data,
            num_samples,
            sample_size,
    ...
    /// Encode a batch of IQP samples via the Native FWT path with FP32 input (PR9c).
    #[cfg(target_os = "linux")]
    pub fn encode_batch_native_f32(
        ...
        crate::profile_scope!("Mahout::EncodeBatchNativeF32");
        ...
        let encoder = match encoding {
            ...
            _ => {
                return Err(MahoutError::InvalidInput(format!(
                    "encode_batch_native_f32 supports iqp and iqp-z only, got {}",
                    encoding_method
                )));
            }
        };
        let state_vector = encoder.encode_batch_native_f32(
            &self.device,
            batch_data,
            num_samples,
            sample_size,
