...
mkdir -p "${REPO}/.venv_wsl/lib"
ln -sf /usr/lib/wsl/lib/libcuda.so.1 "${REPO}/.venv_wsl/lib/libcuda.so"
ln -sf /usr/lib/wsl/lib/libcuda.so.1 "${REPO}/.venv_wsl/lib/libcuda.so.1"
...
export LD_LIBRARY_PATH="${REPO}/.venv_wsl/lib:${NV_LIBS}:${TORCH_LIB}:${LD_LIBRARY_PATH:-}"
